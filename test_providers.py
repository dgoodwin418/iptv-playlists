...
def main():
    ...
    for p, h in sorted(health.items(), key=lambda kv: (-kv[1]["success_rate"], -kv[1]["channel_count"], kv[0].lower())):
        ...
        filename = re.sub(r"[^A-Za-z0-9._-]+", "_", p or "").strip("._-") or "unknown"
        link = "../playlists/by-provider/" + urllib.parse.quote(filename + ".m3u")
        lines.append(f"| `{p}` | {h['channel_count']} | {h['tested_channels']} | {h['passed']} | {h['success_rate']}% | {h['auto_status']} | {lat} | [View Playlist]({link}) |")
